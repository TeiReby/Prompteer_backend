...
### 주요 변경 사항 요약
1. **타이틀 및 프로젝트 개요 추가**: 저장소 진입 시 어떤 서비스인지 1초 만에 알 수 있도록 `# Prompteer Backend`와 한 줄 소개 추가.
2. **사담 정돈**:
   * `(당연히 docker cli가 있어야겠죠)` $\rightarrow$ `(사전 요구사항: Docker 환경)`
   * `심심할때 해보기` $\rightarrow$ `코드베이스 메트릭 측정 (라인 수 / 바이트 수)`
3. **가독성 표준화**: `uv`, 실행 순서, 테스트(Mock/Live), 확장 로드맵을 GitHub 표준 마크다운(코드 블록 및 넘버링)으로 정렬하여 체류 시간 1~3분의 면접관이 즉시 구조를 파악할 수 있도록 최적화.
